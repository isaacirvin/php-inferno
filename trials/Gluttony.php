...
class Gluttony
{
	...
	public function strings_can_contain_many_things()
	{
		...
		assert_that($ciacco)->contains_string("Giacomo");
	}
	...
	public function you_can_build_them_up__()
	{
		...
		// a quirk of implode
		assert_that(implode(' and ', ['hog', 'pig', 'glutton']) == $words)->is_equal_to(true);
	}
	...
	*/
	private function whats_in_the_mouths_of_cerberus($mouth_contents)
	{
        ...
        $mudPattern = '/.*mud.*/';
        $hcPattern = '/.*honey cake.*/';
        $bonesPattern = '/.*bones.*/';

        $contents = [];

        if (preg_match($hcPattern, $mouth_contents) === 1) {
            array_unshift($contents, 'honey cake');
        }
        if (preg_match($bonesPattern, $mouth_contents) === 1) {
            array_unshift($contents, 'bones');
        }
        if (preg_match($mudPattern, $mouth_contents) === 1) {
            array_unshift($contents, 'mud');
        }
